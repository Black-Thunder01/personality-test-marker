...
def _human_friendly_map_function(final_score:int) -> str:
    if final_score >= 5:
        return "high resilient"
    if final_score < 3:
        return "low resilient"
    return "medium resilient"


def append_human_friendly_test_score(df,BRS,column_list):
    """
    Resilient: >= 5
    Low resilience / risk for stress and anxiety: < 3
    """
    df[f"human_{BRS}"] = df[BRS].map(_human_friendly_map_function)
